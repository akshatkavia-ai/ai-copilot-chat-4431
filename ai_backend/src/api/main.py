...
class ChatRequest(BaseModel):
    ...
    session_id: Optional[str] = None # Not used in this implementation, but included for future use
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if not response.parts:
            raise HTTPException(status_code=500, detail="Received an empty response from the AI model.")
